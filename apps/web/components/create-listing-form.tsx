...
export function CreateListingForm({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of toAdd) {
      try {
        ...
        setImages((prev) =>
          prev.map((item) =>
            item.file === file ? { ...item, url, uploading: false } : item
          )
        )
      } catch {
        setImages((prev) =>
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success("Anuncio publicado com sucesso!")
      router.push(`/listings/${listing.id}`)
    } catch (err) {
      const message = err instanceof Error ? err.message : "Erro ao criar anuncio."
      setError(message)
      toast.error(message)
      setIsPending(false)
    }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Input
          ...
          required
          maxLength={255}
          placeholder="Ex: Resina composta 3M, kit completo"
          value={title}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          maxLength={10000}
          rows={5}
          placeholder="Descreva o produto: estado de conservacao, marca, quantidade, etc."
          value={description}
